...
from .auth_method import AuthMethod, HeaderAuthMethod
...
def canonical_path(
    ...
    if len(patterns) == 0:
        raise UrlError(
            f"URL {url} does not match any canonical API path "
...
def endpoint_matches(endpoint_pattern: str, method: str, path: CanonicalPath) -> bool:
    ...
    """
    return (
        endpoint_pattern.startswith(method.upper()) or endpoint_pattern.startswith("*")
...
def entity_wrappers(
    ...
    m = method.upper()
    endpoint = "%s %s" % (m, path)
    match = list(filter(lambda k: endpoint_matches(k, m, path), wrapper_config.keys()))
    ...
    if wrapper is not None:
        ...
        if bod_type is dict:
            if wrapper in body:
                ...
                raise ServerHttpError(error_msg + f"its keys are: {keys}", response)
        else:
            raise ServerHttpError(error_msg + f"its type is {bod_type}.", response)
    else:
...
class RestApiV2BaseClient(ApiClient):
    ...
    def auth_type_mapping(self) -> dict:
        ...
        return {
            ...
            "passthru": PassThruHeaderAuthMethod,
        }
