...
class OAuthTokenClientTest(unittest.TestCase):
    ...
    def test_get_new_token(self, request):
        ...
        self.assertTrue(
            abs(
                864000
                - datetime_to_relative_seconds(response["expiration_date"])
            )
            < 1
        )
        ...
        self.assertEqual(
            "https://identity.pagerduty.com/oauth/token", calls[0][1][1]
        )
        self.assertEqual(
            {
                "client_id": client_id,
    ...
    def test_refresh_client_perform_refresh(self, get_refreshed_token):
        ...
        expiration_date = relative_seconds_to_datetime(
            not_far_enough_in_future
        )
        expiration_date_new = relative_seconds_to_datetime(
            freshly_made_expires_in
        )
